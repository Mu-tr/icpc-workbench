...
- **同步可观测性**：每次平台同步（成败皆记）写入任务历史——拉取数、去重数、限速等待、耗时与可解释的失败分类（凭据失效 / 限流 / 页面结构变化 / 需手动导入等）；`GET /api/sync/diagnostics` 可导出纯文本诊断报告（不含任何密钥），`POST /api/sync/:platform` 支持 `days` 参数仅同步最近 N 天
...
| AtCoder | ✅ | 社区 API `kenkoooo.com` v3 | kenkoooo IRT `difficulty`（−10000…4383，社区模型非官方）→ 按实测锚点分段线性映射 | 支持增量（from_second）；题目资源 24h 磁盘缓存；官方要求页间 ≥1s；可选「用洛谷镜像补标签」（默认关，覆盖有限） |
...
- **AtCoder 的「用洛谷镜像补标签」覆盖有限**：标签来自第三方（洛谷镜像题）归属，默认关闭；实测 250 行样本中 139 行命中题号、其中仅 68 行真的带标签，不作为完整标签来源。
